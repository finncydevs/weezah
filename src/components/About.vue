<script setup></script>

<template>
  <section id="about" class="pt-46 pb-32 bg-slate-900">
    <div class="container">
      <div class="flex flex-wrap">
        <!-- Left Section: About Me -->
        <div class="w-full px-4 mb-10 lg:w-1/2">
          <h4 class="font-bold uppercase text-primary text-lg mb-3">
            Tentang Saya
          </h4>
          <h2 class="font-bold text-dark text-3xl mb-5 max-w-md lg:text-4xl">
            Ayo belajar JavaScript
          </h2>
          <p class="font-medium text-base text-sky-600 max-w-md">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Quaerat
            commodi labore in numquam natus, doloribus officia ducimus, pariatur
            distinctio cupiditate laborum dolor saepe omnis excepturi?
            Obcaecati, dicta. Nam, dolorum commodi?
          </p>
        </div>

        <div class="w-full px-4 lg:w-1/2">
          <h3 class="font-bold text-dark text-3xl mb-4 lg:text-3xl pt-10">
            Add Saya!
          </h3>
          <p class="font-medium text-base text-sky-600 mb-6 lg:text-lg">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Est unde
            modi deleniti at fuga. Maiores omnis fugiat adipisci deleniti itaque
            dolor consequatur corporis sed ipsa consectetur nam iusto,
            necessitatibus exercitationem!
          </p>
          <div class="flex items-center">
            <a
              href="#"
              class="group w-10 h-10 mr-3 rounded-full flex justify-center items-center border-2 border-sky-200 hover:border-primary hover:bg-primary hover:text-dark"
            >
              <i
                class="pi pi-youtube text-2xl group-hover:text-white text-sky-300"
              ></i>
            </a>
            <a
              href=""
              class="group w-10 h-10 mr-3 rounded-full flex justify-center items-center border-2 border-sky-200 hover:border-primary hover:bg-primary hover:text-white"
            >
              <i
                class="pi pi-discord text-2xl fill-current group-hover:text-white text-sky-300"
              ></i>
            </a>
            <a
              href="#"
              class="group w-10 h-10 mr-3 rounded-full flex justify-center items-center border-2 border-sky-200 hover:border-primary hover:bg-primary hover:text-white"
            >
              <i
                class="pi pi-twitter text-xl fill-current group-hover:text-white text-sky-300"
              ></i>
            </a>
            <a
              href="https://www.facebook.com/profile.php?id=100071960914104"
              class="group w-10 h-10 mr-3 rounded-full flex justify-center items-center border-2 border-sky-200 grayscale-100 hover:border-primary hover:bg-primary hover:text-white"
            >
              <i
                class="pi pi-facebook text-2xl fill-current group-hover:text-white text-sky-300"
              ></i>
            </a>
            <a
              href="https://www.github.com/finncydevs/"
              class="group w-10 h-10 mr-3 rounded-full flex justify-center items-center border-2 border-sky-200 hover:border-primary hover:bg-primary hover:text-white"
            >
              <i
                class="pi pi-github text-2xl group-hover:text-white text-sky-300"
              ></i>
            </a>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>
<style scoped>
#about {
  background-image: "https://preview.redd.it/you-should-kill-yourself-now-template-v0-yj3tm7htn4x81.jpg?auto=webp&s=dd9aa2b5387af8e60c2af1e3c5993b2ab8a7f829";
}
</style>
